...
namespace fpnn
{		
	BasicAnswerCallback* ConnectionMap::takeCallback(SOCKET socket, uint32_t seqNum)
	{
	...
	{
		std::list<SOCKET> timeoutedNodes;
		std::unique_lock<std::mutex> lck(_mutex);
		for (auto& cmp: _connections)
		{
			BasicConnection* connection = cmp.second;
	...
	bool ConnectionMap::sendQuestWithBasicAnswerCallback(SOCKET socket, uint64_t token, FPQuestPtr quest, BasicAnswerCallback* callback, int timeout, bool discardableUDPQuest)
	{
		if (!quest)
			return false;
	...
	FPAnswerPtr ConnectionMap::sendQuest(SOCKET socket, uint64_t token, std::mutex* mutex, FPQuestPtr quest, int timeout, bool discardableUDPQuest)
	{
		if (!quest->isTwoWay())
		{
		...
		std::list<TCPClientConnection*>& connectExpiredConnections)
	{
		...
		//-- Step 1: pick invalid connections & requiring ping connections
		{
			bool isLost;
			int timeout;
			std::list<SOCKET> invalidSockets;
