...
{
	//-- If the last Acks has not all been sent, for example, MTU 1500, but there are more than 400 seqs
	//-- that need to be confirmed. So you need to determine that condition: receivedSeqs.size() > 0.
	...
	_sendToken(true), _recvToken(true), _mutex(mutex), /*_unaIncludeIndex(0),*/ _lastUrgentMsec(0)
{
	...
	_packageIdNumber = (uint16_t)slack_real_msec();
...
*/
	...
	_UDPSeqBase = (uint32_t)slack_real_msec();
}
...
bool UDPIOBuffer::prepareSegmentedDataSection(int sectionCount)
{
	...
	//-- Order for: unsigned number + remainedSpaceCorrectionSize, avoid calculation overflow.
...
bool UDPIOBuffer::prepareDataSection(int sectionCount)
{
	...
	if (segmented && sectionCount > 0)
	{
		...
		if (dataUnit->data->length() <= fullSinglePackageSpace)
			return false;				//-- The next package will send as a whole package without segmentation, which simplifies the receiver's processing.
	}
...
bool UDPIOBuffer::prepareSendingPackage(bool& blockByFlowControl)
{
	...
	if (!_sendingAdjustor.sendingCheck())		//-- TODO：This maybe can be removed after flow control added.
	{
	...
	if (_resentCount > Config::UDP::_max_resent_count_per_call)		//-- TODO：This maybe can be removed after flow control added.
	{
		blockByFlowControl = true;
		return false;
	}
	...
	if (_unconformedMap.size() >= Config::UDP::_unconfiremed_package_limitation)		//-- TODO：This maybe can be removed after flow control added.
	{
		blockByFlowControl = true;
		return false;
	}
	...
	{
		blockByFlowControl = true;
		return false;		//-- flow control blocking sending.
	}
...
void UDPIOBuffer::realSend(LPWSAOVERLAPPED overLapped, bool& realSent, bool& blockByFlowControl)
{
	...
	_wsaSendBuffer.len = _currentSendingBuffer.dataLength;
